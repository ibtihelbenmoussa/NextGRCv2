...
interface MacroProcessesIndexProps {
    macroProcesses: PaginatedData<MacroProcess>;
    stats: {
        total: number;
        active: number;
        processes: number;
...
export default function MacroProcessesIndex({
    macroProcesses,
    stats,
    managers,
    businessUnits,
    ...
    const handleExport = async () => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                link.href = url;
                link.download = `macro-processes-${new Date().toISOString().split('T')[0]}.xlsx`;
    ...
    const columns: ColumnDef<MacroProcess>[] = [
        ...
        },
        {
            id: 'actions',
            cell: ({ row }) => {
                const macroProcess = row.original;
                return (
                    <DropdownMenu>
    ...
    const handleDeleteConfirm = () => {
        if (macroProcessToDelete) {
            router.delete(`/macro-processes/${macroProcessToDelete.id}`, {
                onSuccess: () => {
                    setDeleteDialogOpen(false);
                    setMacroProcessToDelete(null);
                },
            });
        }
    };
