...
export default function BusinessUnitsIndex({
    ...
    const handleExport = async () => {
        ...
        try {
            ...
            const response = await fetch(`/business-units/export?${params.toString()}`, {
                method: 'GET',
                headers: {
    ...
    const columns: ColumnDef<BusinessUnit>[] = [
        ...
        },
        {
            ...
            cell: ({ row }) => {
                ...
                const maxVisible = 3;
                const visibleManagers = mgrs.slice(0, maxVisible);
                const extraCount = mgrs.length - maxVisible;
                return (
        ...
        },
        {
            ...
            header: ({ column }) => (
                <DataTableColumnHeader column={column} title="Macro Processes" />
            ),
            meta: { title: 'Macro Processes' },
            cell: ({ row }) => (
                ...
                return (
                    <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                            ...
                            <DropdownMenuItem
                                onClick={() => router.visit(`/business-units/${businessUnit.id}`)}
                            >
                            ...
                            {can('business-units.edit') && (
                                <DropdownMenuItem
                                    onClick={() => router.visit(`/business-units/${businessUnit.id}/edit`)}
                                >
    ...
    return (
        <AppLayout breadcrumbs={[{ title: 'Business Units', href: '/business-units' }]}>
            ...
            <div className="space-y-6 p-4">
                ...
                <div className="flex items-center justify-between">
                    ...
                    {can('business-units.create') && (
                        <Button onClick={() => router.visit('/business-units/create')}>
                ...
                <div className="grid gap-3 md:grid-cols-3">
                    <div className="rounded-lg border bg-card p-4">
                        <div className="flex items-center justify-between">
                            <div className="space-y-1">
                                <p className="text-xs font-medium text-muted-foreground">Total Business Units</p>
                    ...
                    <div className="rounded-lg border bg-card p-4">
                        <div className="flex items-center justify-between">
                            <div className="space-y-1">
                                <p className="text-xs font-medium text-muted-foreground">Macro Processes</p>
                                <div className="flex items-baseline gap-2">
                                    <p className="text-2xl font-bold">{stats.macro_processes}</p>
                                    {stats.total > 0 && (
                                        <span className="text-xs text-muted-foreground">
                                            {(stats.macro_processes / stats.total).toFixed(1)} avg
                                        </span>
                                    )}
                                </div>
                            </div>
            ...
            <AlertDialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
                <AlertDialogContent>
                    <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                            Are you sure you want to delete "{businessUnitToDelete?.name}"?
