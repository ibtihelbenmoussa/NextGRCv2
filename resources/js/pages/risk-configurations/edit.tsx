...
export default function EditRiskConfiguration({ configuration }: Props) {
    ...
    const initialScoreLevels = configuration?.score_levels?.length
        ...
        : autoGenerateScoreLevels(3, 25);

    const [useCriterias, setUseCriterias] = useState(configuration?.use_criterias ?? false);
    const [scoreLevelCount, setScoreLevelCount] = useState<number>(initialScoreLevels.length);
    const [scoreLevels, setScoreLevels] = useState<RiskScoreLevel[]>(initialScoreLevels);
    ...
    const { data, setData, processing, errors } = useForm<{
        ...
        impact_scale_max: configuration?.impact_scale_max ?? 5,
        probability_scale_max: configuration?.probability_scale_max ?? 5,
        calculation_method: configuration?.calculation_method ?? 'max',
        use_criterias: configuration?.use_criterias ?? false,
        impacts: configuration?.impacts ?? [],
        probabilities: configuration?.probabilities ?? [],
        criterias: configuration?.criterias ?? [],
    ...
    }, [currentStep, data, scoreLevels, totalMaxScore]);
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        router.put(`/risk-configurations/${configuration.id}`, payload as any, {
            preserveScroll: true,
    ...
    return (
        <AppLayout
            breadcrumbs={[
                ...
                { title: 'Edit', href: `/risk-configurations/${configuration.id}/edit` },
            ]}
        >
                                    ...
                                    {idx === 0 && (
                                        <Card className="border border-slate-200 dark:border-slate-700 rounded-lg p-3 sm:p-4">
                                            <CardHeader className="p-0 mb-3">
                                                ...
                                                <CardDescription className="text-slate-600 dark:text-slate-400 text-sm leading-relaxed">Update the fundamental parameters for your risk configuration</CardDescription>
                                            </CardHeader>
                                        ...
                                        <Card className="border border-slate-200 dark:border-slate-700 rounded-lg p-3 sm:p-4">
                                            <CardHeader className="p-0 mb-3">
                                                ...
                                                <CardDescription className="text-slate-600 dark:text-slate-400 text-sm leading-relaxed">
                                                    Edit the impact levels and their corresponding scores.
                                    ...
                                    {idx === 3 && (
                                        <Card className="border border-slate-200 dark:border-slate-700 rounded-lg p-3 sm:p-4">
                                            <CardHeader className="p-0 mb-3">
                                                ...
                                                <CardDescription className="text-slate-600 dark:text-slate-400 text-sm leading-relaxed">
                                                    Edit the probability levels and their corresponding scores.
                                                    <span className="ml-1 text-rose-600 dark:text-rose-400 font-medium">Scores must be strictly increasing.</span>
                                                </CardDescription>
                                            </CardHeader>
                                            <CardContent className="p-0 pt-2">
                                    ...
                                    {step.title === 'Assessment Criteria' && (
                                        <Card className="border border-slate-200 dark:border-slate-700 rounded-lg p-3 sm:p-4">
                                            <CardHeader className="p-0 mb-3">
                                                ...
                                                <CardDescription className="text-slate-600 dark:text-slate-400 text-sm leading-relaxed">Edit criteria for multi-dimensional risk assessment</CardDescription>
                                            </CardHeader>
                                            <CardContent className="p-0 pt-2">
                        ...
                        <div className="sticky bottom-0 bg-white/80 dark:bg-slate-900/80 backdrop-blur-md border-t border-slate-200 dark:border-slate-700 py-3 sm:py-4 px-3 sm:px-6 flex flex-col sm:flex-row justify-between items-center gap-3 sm:gap-0">
                            ...
                            <div className="flex gap-2 w-full sm:w-auto order-1 sm:order-2">
                                {currentStep < dynamicSteps.length ? (
                                    ...
                                    >
                                        {processing ? (
                                            <><LoaderCircle className="mr-2 size-3 animate-spin" />Saving...</>
                                        ) : (
                                            <><Check className="mr-2 size-3" />Save Changes</>
                                        )}
                                    </Button>
                                )}
                            </div>
