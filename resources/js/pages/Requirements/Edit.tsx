...
export default function EditRequirement() {
  ...
  const addFiles = (files: FileList | File[]) => {
    ...
    const next: DocEntry[] = arr.slice(0, remaining).map((file) => ({
      ...
      category:    '',
      description: '',
      error: file.size > MAX_SIZE_MB * 1024 * 1024 ? `File exceeds ${MAX_SIZE_MB} MB limit` : undefined,
    }))
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    router.post(route('requirements.update', requirement.id), formData, {
      ...
      onError:   () => { setFlash({ type: 'error', message: 'Error updating requirement. Please check the form fields.' }); setFlashOpen(true) },
    })
  }
      ...
      <Dialog open={!!confirmDeleteId} onOpenChange={() => setConfirmDeleteId(null)}>
        <DialogContent>
          ...
          <p className="py-2 text-sm text-muted-foreground">
            <span className="font-medium text-foreground">{docToDelete?.file_name}</span>{' '}
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <Card className="border shadow-sm">
            ...
            <CardContent className="pt-6 space-y-6">
                ...
                <div className="grid gap-3 sm:grid-cols-3">
                  {COMPLIANCE_LEVELS.map((level) => {
                    ...
                    return (
                      ...
                      >
                        {isSelected && (
                          <CheckCircle2 className={cn('absolute top-3 right-3 h-4 w-4', colors.icon)} />
                        )}
              ...
              {/* Effective Date — éditable en mode edit contrairement au create */}
              <div className="space-y-1.5">
                ...
                {errors.effective_date
                  ...
                  : <p className="flex items-center gap-1.5 text-xs text-muted-foreground">
                      ...
                      You can adjust the effective date manually in edit mode.
                    </p>
                }
              </div>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <CardDescription className="text-xs">
                    Attach files related to this requirement (optional, max {MAX_FILES} files, {MAX_SIZE_MB} MB each)
                  </CardDescription>
                </div>
              </div>
            </CardHeader>
            ...
            <CardContent className="pt-6 space-y-6">
                  ...
                  {existingDocs.map((doc) => (
                    <div key={doc.id} className="flex items-center gap-3 rounded-lg border bg-muted/20 px-4 py-3">
                      ...
                      <div className="flex items-center gap-1 shrink-0">
                        <a
                          href={route('requirements.documents.download', { requirement: requirement.id, document: doc.id })}
              ...
              {docs.length >= MAX_FILES && (
                <p className="flex items-center gap-1.5 text-xs text-amber-600 dark:text-amber-400">
                  <AlertCircle className="h-3.5 w-3.5" />Maximum of {MAX_FILES} files reached.
                </p>
              )}
            </CardContent>
          </Card>
