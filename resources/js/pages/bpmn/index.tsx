...
export default function BPMNIndex({
    ...
    const [diagramToDelete, setDiagramToDelete] = useState<BPMNDiagram | null>(null);
    const [previewDialogOpen, setPreviewDialogOpen] = useState(false);
    const [diagramToPreview, setDiagramToPreview] = useState<BPMNDiagram | null>(null);
    ...
    const columns: ColumnDef<BPMNDiagram>[] = [
        ...
        },
        {
            ...
            meta: { title: 'Associated With' },
            header: ({ column }) => (
                <DataTableColumnHeader column={column} title="Associated With" />
            ),
            cell: ({ row }) => (
                <a
                ...
                >
                    {row.getValue('diagramable_name') || row.original.diagramable_type}
                </a>
            ),
        },
        {
                ...
                return (
                    <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                            ...
                            <DropdownMenuItem
                                onClick={() => router.visit(`/bpmn-diagrams/${diagram.id}`)}
                            >
                            ...
                            {can('bpmn.update') && (
                                <DropdownMenuItem
                                    onClick={() => router.visit(`/bpmn-diagrams/${diagram.id}/edit`)}
                                >
            ...
            <div className="space-y-6 p-4">
                ...
                <div className="flex items-center justify-between">
                    <div>
                        ...
                        <p className="text-muted-foreground">
                            Manage BPMN diagrams for processes and macro processes
                        </p>
                    </div>
                    {can('bpmn.create') && (
                        <Button onClick={() => router.visit('/bpmn-diagrams/create')}>
                ...
                <div className="grid gap-3 md:grid-cols-3">
                    <div className="rounded-lg border bg-card text-card-foreground shadow-sm">
                        <div className="p-4">
                            <p className="text-sm font-medium text-muted-foreground">Total Diagrams</p>
                            <p className="text-2xl font-bold">{statsData.total}</p>
                        </div>
                    </div>
                    <div className="rounded-lg border bg-card text-card-foreground shadow-sm">
                        <div className="p-4">
                            <p className="text-sm font-medium text-muted-foreground">Process Diagrams</p>
                            <p className="text-2xl font-bold">{statsData.process_diagrams}</p>
                        </div>
                    </div>
                    <div className="rounded-lg border bg-card text-card-foreground shadow-sm">
                        <div className="p-4">
                            <p className="text-sm font-medium text-muted-foreground">Macro Process Diagrams</p>
                            <p className="text-2xl font-bold">{statsData.macro_process_diagrams}</p>
                        </div>
                    </div>
                </div>
                ...
                {isLoading ? (
                    <div className="flex h-32 items-center justify-center">
                        <p className="text-muted-foreground">Loading BPMN diagrams...</p>
                    </div>
                ) : (
                    <ServerDataTable
                        columns={columns}
            ...
            <AlertDialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
                <AlertDialogContent>
                    <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                            Are you sure you want to delete "{diagramToDelete?.name}"?
                    ...
                    <ScrollArea className="flex flex-col justify-between overflow-hidden">
                        <DialogHeader className="contents space-y-0 text-left">
                            ...
                            <DialogDescription asChild>
                                <div className="p-6">
                                    {diagramToPreview && diagramToPreview.bpmn_xml ? (
                                        <BpmnViewerWithProperties xml={diagramToPreview.bpmn_xml} />
                                    ) : (
